    ...
    for dir_name in sorted(files_by_dir):
        for xml_path in sorted(files_by_dir[dir_name]):
            ...
            if not m:
                continue
            tid = m.group(1)
            # If the id contains macro tokens (e.g. bcftools_@EXECUTABLE@),
            # derive the real tool ID from the filename. Each XML file in
            # such a suite is a distinct tool even though they share the
            # same unexpanded id attribute.
            if '@' in tid:
                tid = xml_path.stem  # e.g. bcftools_annotate.xml → bcftools_annotate
            # Unique key is (directory, filename) so tools with the same
            # id in different files are both tracked.
            key = f"{dir_name}/{xml_path.name}"
            if key in seen_keys:
                continue
            seen_keys.add(key)
