...
def run(
    ...
    """Programmatic entry point. Returns the list of ToolEvent objects.

    Vendored from PR #8152 (which will not be merged) — this website replaces it.
    Plots are skipped by default (the site renders interactive Vega-Lite charts
    from the emitted tool_events.json instead of static matplotlib PNGs).
    """
    output = Path(output_dir)
    output.mkdir(parents=True, exist_ok=True)
    tools = discover_tools(repo_path=repo, tools_path=tools_path)
    if not tools:
        return []
    events = analyze_tools(
        repo_path=repo, tools=tools, branch=branch,
        tools_path=tools_path, verbose=verbose, workers=workers,
    )
    if fmt in ("csv", "both"):
        write_events_csv(events, str(output / "tool_events.csv"))
    if fmt in ("json", "both"):
        write_events_json(events, str(output / "tool_events.json"))
    write_summary(events, str(output / "summary.txt"))
    if not skip_plots:
        from .plots import generate_all_plots
        generate_all_plots(events, str(output))
    return events
